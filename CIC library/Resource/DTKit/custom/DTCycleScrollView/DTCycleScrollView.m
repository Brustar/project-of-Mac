...
//    if (self.delegate && [self.delegate respondsToSelector:@selector(scrollView:didEndDecelerating:)]) {
//        [self.delegate scrollView:self didEndDecelerating:[self.curViews objectAtIndex:0]];
//    }
    if (self.delegate && [self.delegate respondsToSelector:@selector(scrollView:didEndSelect:)]) {
        [self.delegate scrollView:self didEndSelect:[self.curViews objectAtIndex:0]];
    }
...
- (void)next
{
    ...
    [self.scrollView setContentOffset:CGPointMake(point.x+CGRectGetWidth(self.scrollView.frame), point.y) animated:YES];
//    if (self.delegate && [self.delegate respondsToSelector:@selector(scrollView:didEndDecelerating:)]) {
//        [self.delegate scrollView:self didEndDecelerating:[self.curViews objectAtIndex:2]];
//    }
    if (self.delegate && [self.delegate respondsToSelector:@selector(scrollView:didEndSelect:)]) {
        [self.delegate scrollView:self didEndSelect:[self.curViews objectAtIndex:2]];
    }
}
...
//    [self.scrollView setContentOffset:CGPointMake(self.scrollView.frame.size.width, 0) animated:YES];
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(scrollView:didEndDecelerating:)]) {
        [self.delegate scrollView:self didEndDecelerating:[self.curViews objectAtIndex:1]];
    }
}
